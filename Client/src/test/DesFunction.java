...
public class DesFunction{
    ...
    /*  轮函数  */
	public static int[] function(int[] s,int[] Key)
	{
	...
	/*  用E表扩展 ，32bit->48bit */
	public static int[] Expand(int[] s)
	{
		int[] temp = new int[48];
		for(int j = 0;j < 48;j ++)
		{
	...
	/*  异或操作（模2加法）  */
	public static int[] add(int[] M,int[] Key)
	{
		int[] temp = new int[48];
    ...
    /*  用S表进行代换，48bit->32bit  */
	public static int[] Narrow(int[] M)
	{
		int[] temp = new int[32];
		int X;
		int Y;
		for(int j = 0,k = 0,l = 0;j < 43; j = j+6,l ++)
		{
			X = M[j]*2+M[j+5];
			Y = M[j+1]*8+M[j+2]*4+M[j+3]*2+M[j+4];
			int a = S[l][X][Y];
			temp[k] = a/8;
			temp[k+1] = (a%8)/4;
			temp[k+2] = (a%4)/2;
    ...
    /*  用P表进行置换  */
	public static int[] Replace(int[] M)
	{
		int[] tem = new int[32];
		int[] temp = new int[32];
		tem = (int[])M.clone();
